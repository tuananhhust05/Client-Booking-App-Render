        ...
        if(response && response.data && response.data.data){
           ...
           axios.post(`${url()}/hotels/TakeDataCity`,{listCity:arrCity}).then((response2)=>{
               if(response2 && response2.data && response2.data.data){
                    ...
                    for(let i=0; i< response.data.data.length ; i++){
                        let imgLink = "";
                        if(dataCity.find((e)=> String(e.name) === response.data.data[i]._id)){
                          imgLink = dataCity.find((e)=> String(e.name) === response.data.data[i]._id).img;
                        }
                        let a = {
                    ...
                    else if(Number(dataFinal.length) === 1){
              ...
              <div className = "form_show_more_wrapper">
                  {
                    dataHotelAll.map((obj,index)=>(
                        <div key={index} 
                            ...
                            <img className="ele_form_show_more_img" src={obj.img} onError={imageOnErrorHotel} alt={obj.img}/>
